...
\begin{tikzpicture}[scale=0.62, transform shape, >=Stealth]
  ...
  \draw[dotted, gray] (-1.2,-2) -- (-1.2,2) node[above]{$\delta$ cst};
\end{tikzpicture}
\end{center}
\vars{$\cos\theta = \zeta$, $|s| = \omega_n$, Re$(s) = -\delta = -\zeta\omega_n$, Im$(s) = \pm\omega_0$. Iso-$\zeta$ = droites. Iso-$\delta$ = verticales. Iso-$\omega_n$ = cercles.}
\end{tcolorbox}
...
\toprule
$\zeta$ & $D_\%$ & $\varphi_m$ approx. & $Q=1/(2\zeta)$ & $t_{s5}\omega_n$ & Commentaire \\
\midrule
