...
\begin{tcolorbox}[colback=ch3green!4,colframe=ch3green!80!black,title={Syst\`eme du 1\textsuperscript{er} ordre}]
\tightmath{
...
\end{tabular}
\end{center}
\vars{$t_s(5\%) \approx 3\tau$, $t_s(2\%) \approx 4\tau$. Pente init. $= Ku_0/\tau$. Pas de d\'epassement. R\'ep. impuls. : $y(t) = \frac{K}{\tau}e^{-t/\tau}$.}
\end{tcolorbox}

\begin{tcolorbox}[colback=ch3green!2,colframe=ch3green!80!black,title={1\textsuperscript{er} ordre : rampe \& identification}]
...
\begin{tcolorbox}[colback=ch3green!4,colframe=ch3green!80!black,title={Syst\`eme du 2\textsuperscript{nd} ordre}]
\tightmath{
...
\tightmath{
\boxed{\omega_n = \sqrt{\sigma^2+\omega_0^2}} \quad \boxed{\zeta = \frac{\sigma}{\omega_n} = \frac{-\Re(s)}{\omega_n}}
}
\tightmath{
...
$0{<}\zeta{<}1$ & \textbf{Sous-amorti} & $-\sigma \pm j\omega_0$ & Oscil. amorties \\
...
\begin{tcolorbox}[colback=ch3green!2,colframe=ch3green!80!black,title={Cas sous-amorti ($0 < \zeta < 1$) : formules cl\'es}]
\tightmath{
\boxed{\omega_0 = \omega_n\sqrt{1{-}\zeta^2}} \quad \sigma = \zeta\omega_n \quad \varphi = \arccos(\zeta)
}
R\'eponse indicielle :
\tightmath{
y(t) = K\!\left[1 - \frac{e^{-\sigma t}}{\sqrt{1{-}\zeta^2}}\sin(\omega_0 t + \varphi)\right]
}
\tightmath{
...
\piege{$\zeta \approx 0.7 \Rightarrow D_\% \approx 5\%$ : bon compromis rapidit\'e/stabilit\'e.}
\begin{center}
\renewcommand{\arraystretch}{1}
\scriptsize
...
\midrule
$D_\%$ & 73 & 53 & 37 & 25 & 16 & 9.5 & 4.6 & 1.5 \\
\bottomrule
\end{tabular}
\end{center}
\end{tcolorbox}

\begin{tcolorbox}[colback=ch3green!4,colframe=ch3green!80!black,title={Position des p\^oles dans le plan $s$}]
\begin{center}
\begin{tikzpicture}[scale=0.62, transform shape, >=Stealth]
  ...
  \filldraw[ch3green!80!black] (-1.2,-1.5) circle (2pt) node[right]{$\bar{s}_1$};
  \draw[dashed, gray, thin] (0,0) -- (-1.2,1.5);
  \draw[<->, thin, red!70] (-1.2,0) -- (-1.2,1.5) node[midway, right]{$\omega_0$};
  \draw[<->, thin, red!70] (0,0) -- (-1.2,0) node[midway, below]{$\sigma$};
  \node at (-0.3,1.0) {$\omega_n$};
  \draw[thin] (-0.35,0) arc (180:128:0.35); \node at (-0.55,0.28) {$\theta$};
  \draw[dotted, gray] (0,0) -- (-2.5,2.1) node[above]{$\zeta$ cst};
  \draw[dotted, gray] (-1.2,-2) -- (-1.2,2) node[above]{$\sigma$ cst};
\end{tikzpicture}
\end{center}
\vars{$\cos\theta = \zeta$, $|s| = \omega_n$, Re$(s) = -\sigma = -\zeta\omega_n$, Im$(s) = \pm\omega_0$. Iso-$\zeta$ = droites. Iso-$\sigma$ = verticales. Iso-$\omega_n$ = cercles.}
\end{tcolorbox}

\begin{tcolorbox}[colback=ch3green!2,colframe=ch3green!80!black,title={Cas critique, sur-amorti \& identification}]
...
\vars{\textbf{P\^ole dominant} = plus petite $|\text{Re}|$ (le plus lent, $\tau$ le plus grand). Approx. : $G \approx K/(1{+}\tau_{\max}s)$ si rapport $\tau_1/\tau_2 \geq 5$.}
...
\begin{tcolorbox}[colback=ch3green!4,colframe=ch3green!80!black,title={Abaque rapide des performances (2\textsuperscript{nd} ordre)}]
\begin{center}
\renewcommand{\arraystretch}{1.02}
\scriptsize
\begin{tabular}{@{}cccccc@{}}
\toprule
$\zeta$ & $D_\%$ & $MP$ approx. & $Q=1/(2\zeta)$ & $t_{s5}\omega_n$ & Commentaire \\
\midrule
0.2 & 53\% & $\sim 20°$ & 2.5 & 15 & Tr\`es oscillant \\
0.4 & 25\% & $\sim 40°$ & 1.25 & 7.5 & Oscillant \\
0.5 & 16\% & $\sim 50°$ & 1.0 & 6 & Compromis \\
0.7 & 4.6\% & $\sim 65°$ & 0.71 & 4.3 & Bon amort. \\
1.0 & 0\% & $>$70° & 0.5 & 3.0--4.8 & Critique \\
\bottomrule
\end{tabular}
\end{center}
\tightmath{
t_{s2} \approx \frac{4}{\zeta\omega_n},\quad t_{s5} \approx \frac{3}{\zeta\omega_n},\quad t_p=\frac{\pi}{\omega_0}
}
\astuce{Quand l'\'enonc\'e impose un d\'epassement max, commencer par $D_\% \to \zeta$, puis remonter vers $\omega_n$ via $t_s$ ou $t_p$.}
\end{tcolorbox}
