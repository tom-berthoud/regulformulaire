...
\begin{center}
\begin{tikzpicture}[scale=1.02, transform shape, >=Stealth,
   every node/.style={font=\fontsize{6.0}{6.8}\selectfont}]
  % --- omega_co : croisement EXACT du lieu et du cercle unite (3e quadrant) ---
  \coordinate (co) at ({cos(235)},{sin(235)});
  % --- axes ---
  \draw[->] (-2.5,0) -- (1.05,0) node[right]{Re};
  \draw[->] (0,-1.85) -- (0,1.55) node[above]{Im};
  ...
  % --- lieu omega>0 (trait plein) : depart omega->0 a droite, descend, croise le
  %     cercle unite en omega_co, puis l'axe reel negatif en omega_pi (entre -1 et
  %     0 => marges OK), puis -> origine ---
  \draw[very thick, ch6teal!80!black]
     (2.0,-0.12) to[out=-100,in=18] (0.2,-1.18)
     to[out=198,in=-70] (co)
     to[out=110,in=-58] (-0.62,0)
     to[out=125,in=-150] (-0.30,0.5)
     to[out=30,in=170] (0.02,0.05);
  % --- conjugue omega<0 (tiret) ---
  \draw[very thick, ch6teal!80!black, dashed]
     (2.0,0.12) to[out=100,in=-18] (0.2,1.18)
     to[out=162,in=58] (-0.62,0);
  ...
  \draw[blue!75, thin] (-0.62,0) -- (-0.98,-0.40);
  \node[blue!75, left] at (-1.0,-0.42) {$\omega_\pi$};
  ...
  % --- omega_co : marqueur place exactement sur le lieu (= sur le cercle) ---
  \filldraw[orange!85!black] (co) circle (1.7pt);
  \draw[orange!85!black, thin] (co) -- ({1.5*cos(235)},{1.5*sin(235)});
  \node[orange!85!black, below] at ({1.52*cos(235)},{1.56*sin(235)}) {$\omega_{co}$};
  % --- marge de phase : rayon vers omega_co + arc -axe reel -> G_o(jw_co) ---
  \draw[gray!55, thin] (0,0) -- (co);
  \draw[red!70!black, line width=0.9pt] (180:0.55) arc (180:235:0.55);
  \draw[red!70!black, line width=0.3pt] ({0.55*cos(207.5)},{0.55*sin(207.5)}) -- (0.30,-0.5);
  \node[red!70!black, right] at (0.28,-0.52) {$\varphi_m$};
\end{tikzpicture}
\end{center}
