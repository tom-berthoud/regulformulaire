...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={Table de Laplace essentielle}]
\begin{center}
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={Propri\'et\'es de la transform\'ee}]
\begin{center}
\renewcommand{\arraystretch}{1.05}
\scriptsize
\begin{tabular}{@{}ll@{}}
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={Fonction de transfert}]
\tightmath{
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={Trois formes de $G(s)$}]
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={Changement de forme}]
\begin{minipage}[t]{0.48\linewidth}
\textbf{Laplace $\rightarrow$ Bode}

\tightmath{
\frac{K}{s^2 + 2\delta s + \omega_n^2}
= \frac{K/\omega_n^2}{1 + \frac{2\zeta}{\omega_n}s + \frac{s^2}{\omega_n^2}}
}
\tightmath{
\omega_n = \sqrt{\delta^2 + \omega_0^2}
\qquad
\zeta = \frac{\delta}{\omega_n}
}
\vars{Aussi : $(s+a)=a\!\left(1+\frac{s}{a}\right)$, donc $\tau=\frac{1}{a}$ et $\omega_p=a$ (cassure).}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\linewidth}
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={Type $\alpha$, Retard pur, Gain permanent}]
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={Cha\^ine d'action \& boucle ferm\'ee}]
\textbf{Cha\^ine d'action} : $G = G_1G_2\cdots G_n$. \quad
\textbf{Boucle ouverte} : $G_o(s)=G_c\,G_a$ ($G_a$=syst\`eme \`a r\'egler).
...
\end{center}
\vars{Transfert de boucle (BF unitaire) : $G_o(s)=G_c\,G_a$. En cas de doute, repartir de $y=G(u\mp Hy)$.}
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={D\'ecomposition en fractions partielles}]
...
\textbf{P\^oles complexes} $-\delta \pm j\omega$ : garder $\frac{As+B}{(s{+}\delta)^2+\omega^2}$ et utiliser table.
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={Inversion de Laplace : recette d'examen}]
\begin{enumerate}
  ...
  \item Choisir la bonne forme : $\frac{A}{s-p}$, $\frac{A}{(s-p)^2}$, $\frac{As+B}{(s+\delta)^2+\omega^2}$
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={M\'ethodologie : esquisse de la r\'ep. temporelle},top=0.3mm,bottom=0.3mm,before skip=0.8pt,after skip=0.8pt]
...
\begin{itemize}
  ...
  \item Calculer $T_{\text{r\'eg}} = 3/|\delta|$ et $T_0 = 2\pi/\omega_0$
  \item D\'ecaler de $T_r$ et sommer les r\'eponses aux \'echelons successifs
\end{itemize}
\vars{Exemple : $G(s)=\dfrac{10}{(s+2)^2+16}e^{-s}$, p\^oles $-2\pm j4$, $y_\infty=0.5$, $T_{\text{r\'eg}}=1.5$ s, $T_0\approx1.57$ s, $T_r=1$ s.}
\begin{center}
...
% pôles: -2 ± j4, omega_d=4, sigma=2, y_inf = 0.5
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={Identification d'un syst\`eme oscillant}]
\tightmath{
...
\begin{tcolorbox}[colback=ch2orange!5,colframe=ch2orange!80!black,title={Lecture d'un diagramme de Bode}]
