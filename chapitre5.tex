...
$a_3 > 0$, $b_1 = a_3 a_2 - a_1 > 0$, et $b_1 a_1 - a_3 a_0 > 0$.
\end{tcolorbox}

\begin{tcolorbox}[colback=ch5red!2,colframe=ch5red!80!black,title={Stabilit\'e en boucle ferm\'ee}]
\'Equation caract\'eristique :
\tightmath{
\boxed{1 + G_{BO}(s) = 0} \quad G_{BO} = G_r G_a G_p G_m
}
BF stable $\Leftrightarrow$ toutes les racines de $1 + G_{BO}(s) = 0$ ont Re $< 0$.
\end{tcolorbox}

\begin{tcolorbox}[colback=ch5red!4,colframe=ch5red!80!black,title={Marge de gain (MG)}]
$\omega_\pi$ = \textbf{croisement de phase} : $\angle G_{BO}(\jw_\pi) = -180°$.
\tightmath{
\boxed{MG = \frac{1}{|G_{BO}(\jw_\pi)|}} \quad \boxed{MG_{\text{dB}} = -|G_{BO}(\jw_\pi)|_{\text{dB}}}
}
\vars{Sur Bode : $MG_{\text{dB}}$ = distance module $\leftrightarrow$ 0~dB, \textbf{lue \`a $\omega_\pi$}. Stable si courbe \textbf{en dessous} de 0~dB. Typ. : $MG \geq 6$--$10$ dB.}
\end{tcolorbox}

\begin{tcolorbox}[colback=ch5red!2,colframe=ch5red!80!black,title={Marge de phase (MP)}]
$\omega_c$ = \textbf{croisement de gain} : $|G_{BO}(\jw_c)| = 1$ (0~dB).
\tightmath{
\boxed{MP = 180° + \angle G_{BO}(\jw_c)}
}
\vars{Sur Bode : $MP$ = distance phase $\leftrightarrow$ $-180°$, \textbf{lue \`a $\omega_c$}. Stable si phase \textbf{au-dessus} de $-180°$. Objectif : $30° < MP < 60°$.}
\end{tcolorbox}
...
\begin{tikzpicture}[scale=0.56, transform shape, >=Stealth]
  ...
  \begin{scope}[xshift=6.8cm]
    ...
    \draw[red, thick, dashed] (2.5,-2.2) -- (2.5,-0.8); \node[red, above] at (2.5,-0.75) {$\omega_c$};
    \draw[orange, thick, dashed] (4,-2.2) -- (4,-1.35); \node[orange, above] at (4,-1.3) {$\omega_\pi$};
    \draw[<->, green!60!black] (2.5,-1.57) -- (2.5,-1.1); \node[left, green!60!black] at (2.3,-1.3) {$MP$};
  \end{scope}
\end{tikzpicture}
\end{center}
\vars{$\alpha$ : pente BF du module ; $d$ : pente HF. $\omega_c \Rightarrow MP$, $\omega_\pi \Rightarrow MG$.}
\end{tcolorbox}

\begin{tcolorbox}[colback=ch5red!2,colframe=ch5red!80!black,title={Relations approx. MP -- $\zeta$ -- $D_\%$}]
\tightmath{
\boxed{\zeta \approx \frac{MP}{100°}} \quad (\text{pour } 20° < MP < 70°)
}
\vars{MP $= 45° \Rightarrow \zeta \approx 0.45 \Rightarrow D_\% \approx 20\%$. MP $= 65° \Rightarrow \zeta \approx 0.65 \Rightarrow D_\% \approx 7\%$.}
\end{tcolorbox}

\begin{tcolorbox}[colback=ch5red!4,colframe=ch5red!80!black,title={Approximations BF depuis Bode}]
\textbf{$T_{\text{r\'eg}}$ en BF} (approx. demi-p\'eriode \`a $\omega_c$) :
\tightmath{
\boxed{T_{\text{r\'eg}} \approx \frac{\pi}{\omega_c}}
}
\textbf{Retard pur marginalisant la BF} (consume $MP$ : $\omega_c T_r = MP_{\text{rad}}$) :
\tightmath{
\boxed{T_r^{\max} = \frac{MP_{\text{rad}}}{\omega_c} = \frac{MP° \cdot \pi}{180° \cdot \omega_c}}
}
\piege{$MP$ en \textbf{radians} : $MP_{\text{rad}} = MP° / 57.3$. Ex. : $MP{=}40°$, $\omega_c{=}600$ $\Rightarrow$ $T_r^{\max} = \frac{40\pi}{180\cdot600} \approx 1.2\,\text{ms}$.}
\end{tcolorbox}

\begin{tcolorbox}[colback=ch5red!4,colframe=ch5red!80!black,title={Crit\`ere de Nyquist}]
\textbf{Cas simplifi\'e (BO stable, $P{=}0$)} :
BF stable $\Leftrightarrow$ Nyquist $G_{BO}(\jw)$ \textbf{n'entoure pas} $(-1, 0)$.
...
\begin{enumerate}
  \item Former l'\'equation caract\'eristique : $1 + K\,G_{BO}(s) = 0$
...
\end{enumerate}
\tightmath{
\text{Zone utile en pratique : } MP\in[35^\circ,60^\circ],\quad MG\ge 6\text{ dB}
}
